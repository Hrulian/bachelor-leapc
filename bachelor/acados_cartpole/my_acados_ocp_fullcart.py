# Full cart+pendulum model (identical to my_acados_ocp.py), BUT the learnable
# parameter is the CART POSITION reference (xref0) instead of the pole-angle
# reference (xref1).
#
#   state x = [pos, theta, vel, thetadot]   (nx = 4, full model)
#   input u = [F]                           (nu = 1)
#   learnable = xref0 (cart position reference)
#   theta reference (xref1) is fixed at 0 (upright), everything else non-learnable
#
# Everything (dynamics, cost, constraints, solver options) is byte-for-byte the
# same as my_acados_ocp.py; only create_custom_cartpole_params swaps which
# reference is learnable. Mirrors the function names so it can be bound by
# my_planner_fullcart.py exactly like my_planner.py binds my_acados_ocp.py.
...
def create_custom_cartpole_params(
    ...
    Full model, but the LEARNABLE reference is the cart position (``xref0``); the
    pole-angle reference (``xref1``) is fixed at 0 (upright) and non-learnable.
    ...
    return [
        ...
        # Reference parameters — LEARNABLE is the cart position (xref0)
        AcadosParameter(
            "xref0",
