...
def main():
    ...
    max_reward_steps = 1000  # only calculate reward for first 1000 steps
    ...
    try:
        ...
        while True:
            ...
            if not (isinstance(state, tuple) and len(state) == 4):
                ...
                step_count -= 1  # don't count invalid states
                continue
            ...
            # calculate reward only for first 1000 steps
            if step_count <= max_reward_steps:
                # create state tuple with tripped flag (always False for MPC)
                state_with_tripped = (x, theta, v, thetadot, False)
                reward = compute_reward(state_with_tripped, u_force)
                cumulative_reward += reward
                
                # print progress every 100 steps
                if step_count % 100 == 0:
                    avg_reward = cumulative_reward / step_count
                    print(f"Step {step_count}/{max_reward_steps}: Reward = {reward:.4f}, Cumulative = {cumulative_reward:.2f}, Avg = {avg_reward:.4f}")
            
            # print final reward statistics when reaching max_reward_steps
            if step_count == max_reward_steps:
                avg_reward = cumulative_reward / max_reward_steps
                print("="*60)
                print(f"REWARD TRACKING COMPLETE:")
                print(f"  Total Steps: {max_reward_steps}")
                print(f"  Cumulative Reward: {cumulative_reward:.2f}")
                print(f"  Average Reward: {avg_reward:.4f}")
                print(f"MPC continues running without reward calculation...")
                print("="*60)
