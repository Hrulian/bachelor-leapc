...
def advanced_smooth(x_values, y_values, smoothing_param=0.6, viewport_scale=1000):
    ...
    Args:
        ...
        y_values: Array of y-axis values (stabilization binary)
...
def plot_stabilized_from_csv():
    """Read CSV file and plot stabilization success for 3 seeds."""
    ...
    for col, color, name in zip(['1', '2', '3'], colors, seed_names):
        ...
        total_successes = int(np.sum(stabilized))
        success_pct = (total_successes / len(stabilized)) * 100
        
        print(f"\n{name}:")
        print(f"  Episodes: {len(stabilized)}")
        print(f"  First stabilization: Episode {first_success:.0f}" if first_success else "  First stabilization: Never")
        print(f"  Total successes: {total_successes}/{len(stabilized)} ({success_pct:.1f}%)")
        print(f"  Final smoothed value: {smoothed[-1]:.3f}")
    
    # Styling
    ax.set_xlabel('Episode')
    ax.set_ylabel('Stabilization Rate')
    ...
    # Save as PDF
    output_path = script_dir / "stabilized_comparison.pdf"
